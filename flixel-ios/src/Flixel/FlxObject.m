...
@synthesize originalXPos;
@synthesize originalYPos;
@synthesize moving;
...
- (BOOL) overlapsWithOffset:(FlxObject *)Object withCustomXOffset:(float)customXOffset withCustomYOffset:(float)customYOffset
{
    CGPoint _point = [self getScreenXY];
    float tx = _point.x ;
    float ty = _point.y ;
    _point = [Object getScreenXY];
    if ((_point.x <= tx - Object.width - customXOffset) || (_point.x >= tx + width + customXOffset) || (_point.y <= ty - Object.height - customYOffset) || (_point.y >= ty + height + customYOffset))
        return NO;
    return YES;
}
